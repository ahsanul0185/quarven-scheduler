...
export default function FloorView({ assignments, employees }: FloorViewProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-6">
      <h3 className="text-lg font-light text-slate-700 mb-1">Floor View</h3>
      <p className="text-sm font-light text-slate-400 mb-4 sm:mb-6">Who is on each floor, by day.</p>
      <div className="space-y-4 sm:space-y-6">
        {floors.map((floor) => {
          const days = byFloor.get(floor)!
          return (
            <div key={floor} className="border border-slate-200 rounded-2xl overflow-hidden">
              <div className="bg-[#2C3E50] text-white px-4 sm:px-6 py-2 sm:py-3 font-light text-sm">{floor}</div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 p-4 sm:p-5">
                {Array.from(days.entries()).map(([day, entries]) => (
                  <div key={day} className="bg-[#f2f4f6] rounded-xl p-3 sm:p-4">
                    <div className="text-xs font-light uppercase text-slate-500 mb-2 sm:mb-3 tracking-wide">
